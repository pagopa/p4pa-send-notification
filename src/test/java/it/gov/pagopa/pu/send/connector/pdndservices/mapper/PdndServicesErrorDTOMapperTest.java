...
class PdndServicesErrorDTOMapperTest {

  @Test
  void whenMapThenReturnPuErrorDTO() {
    // Given
    PdndServicesErrorDTO errorDTO = TestUtils.getPodamFactory().manufacturePojo(PdndServicesErrorDTO.class);
    Objects.requireNonNull(errorDTO.getFields());

    // When
    PuErrorDTO result = PdndServicesErrorDTOMapper.map(errorDTO);
    ...
    for (int i = 0; i < fields.size(); i++) {
      ...
      it.gov.pagopa.pu.pdndservices.dto.generated.ErrorFieldDTO expectedErrorFieldDTO = errorDTO.getFields().get(i);
