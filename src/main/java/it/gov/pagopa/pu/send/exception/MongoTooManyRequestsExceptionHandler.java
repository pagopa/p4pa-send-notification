  ...
  protected ResponseEntity<SendNotificationErrorDTO> handleDataAccessException(
    ...
    if (isRequestRateTooLargeException(ex)) {
      ...
      if (ex instanceof DataIntegrityViolationException) {
        return CommonExceptionHandler.handleException(ex, request, HttpStatus.CONFLICT, SendNotificationErrorDTO.CategoryEnum.SEND_NOTIFICATION_CONFLICT);
      } else {
        return CommonExceptionHandler.handleException(ex, request, HttpStatus.INTERNAL_SERVER_ERROR, SendNotificationErrorDTO.CategoryEnum.SEND_NOTIFICATION_GENERIC_ERROR);
      }
    }
  }
